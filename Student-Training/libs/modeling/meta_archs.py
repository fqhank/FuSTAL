...
def calculate_iou_matrix(segs1,segs2):
    matrix = segs1.unsqueeze(1).repeat(1,segs2.shape[0],1).cuda()
    matrix_T = segs2.unsqueeze(1).repeat(1,segs1.shape[0],1).permute(1,0,2).cuda()
    matrix_inter = torch.where(matrix[:,:,1]>matrix_T[:,:,1],matrix_T[:,:,1],matrix[:,:,1]) - torch.where(matrix[:,:,0]>matrix_T[:,:,0],matrix[:,:,0],matrix_T[:,:,0])
    matrix_right = torch.where(matrix[:,:,1]>matrix_T[:,:,1],matrix[:,:,1],matrix_T[:,:,1])
    matrix_left = torch.where(matrix[:,:,0]>matrix_T[:,:,0],matrix_T[:,:,0],matrix[:,:,0])
    
    matrix_inter[matrix_inter<0] = 0
    iou = matrix_inter / (matrix_right - matrix_left + 1e-9)
    return iou
...
class TriDet(nn.Module):
    ...
    def get_video_cls_scores(self, cas, n):
        k_easy = n//5
        sorted_scores, _= cas.sort(descending=True, dim=1)
        topk_scores = sorted_scores[:, :k_easy, :]
        video_scores = nn.Softmax(dim=1)(topk_scores.mean(1))
        return video_scores

    def forward(self, video_list, vid_label=None):
        ...
        # fpn_feats, fpn_masks = [batched_inputs], [batched_masks]
        
        # fpn_feats, fpn_masks = feats, masks
        ...
        if self.training:
            ...
            for i in range(1):
                logits = self.dropout(out_cls_logits[i])
                n = fpn_masks[i].sum()
                video_scores = self.get_video_cls_scores(logits,n)
                loss_cls += self.action_criterion(video_scores,vid_label)
            # print(loss_cls)
            losses['final_loss'] += loss_cls
            ######################### 
            return losses
    ...
    def postprocessing(self, results):
        ...
        for results_per_vid in results:
            ...
            if segs.shape[0] > 0:
                segs = (segs * stride + 0.5 * nframes) / fps # 4,16,30
                # truncate all boundaries within [0, duration]se
                segs[segs <= 0.0] *= 0.0
