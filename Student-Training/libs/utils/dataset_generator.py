...
class NpyFeature(data.Dataset):
    ...
    def get(self, name):
        data, vid_num_seg, sample_idx = self.get_data(name)
        label, temp_anno = self.get_label(name, vid_num_seg, sample_idx)

        return torch.tensor(data).unsqueeze(0), torch.tensor(label).unsqueeze(0), temp_anno, name, vid_num_seg
